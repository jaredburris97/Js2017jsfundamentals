$(document).ready(function(){
	localStorage.clear();
	if(!localStorage.getItem('planets')){								// '!' before localStorage
		$.ajax({
			type: 'GET',
			...
			planets.results.forEach(function(p){
				$("#planets").append('<tr><td>'+ p.name + '</td></tr>') // missing '(' after append
	...
	if(!localStorage.getItem('wookie')){
		$.ajax({
			...
			planets.results.forEach(function(p){
				planetsObj.planets.push(p.name)							//missing .name after 'p'
			})
		})
	}
	...
	if(!localStorage.getItem('starships')){
		$.ajax({
			type: 'GET',
			url: 'https://swapi.co/api/starships/',					// do not need '1' after starships/
	...
	if(!localStorage.getItem('movies')){	//if there's not something in local storage called movies, run and fetch movies from api
		let movieObj = {
			movies : []		// holds all values for movies in local, save for later
		}
		$.ajax({
			type: 'GET',
			...
			films.results.forEach(function(p){
				...
				//setItem holds things in local storage. stringify puts movieObj into string
			localStorage.setItem('movies', JSON.stringify(movieObj))	// missing quote before movies
		})
	}
	...
	let clicker = false					//boolean value to switch button
	$(wookie).on('click',function(){	//user clicks wookie button
		clicker = !clicker				//makes clicker=true
		$(wookie).val("Planet Name")	//changes value of button
		$("#planets").html("")
		if(clicker){
			$(changer).html("Wookie Planets");
			let wookieObj = JSON.parse(localStorage.getItem('wookies'));
			wookieObj.wookies.forEach(function(p){
				$("#planets").append('<tr><td>'+ p + '</td></tr>')		//changed AYYEEO to 'p'
			})
	...
	$(moviebtn).on('click',function(e){
		...
		if(clicked){
			...
			let shipsObj = JSON.parse(localStorage.getItem('starships'));	// changed 'movies' to 'starships'
